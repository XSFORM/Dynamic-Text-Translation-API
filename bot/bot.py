...
# =====================================================================
#  SUBNET POOL — leased /24 management
# =====================================================================
SUBNET_POOL_FILE = "/root/monitor_bot/subnet_pool.json"

def load_subnet_pool() -> dict:
    if os.path.exists(SUBNET_POOL_FILE):
        try:
            with open(SUBNET_POOL_FILE) as f:
                return json.load(f)
        except Exception:
            pass
    return {}

def save_subnet_pool(data: dict) -> None:
    with open(SUBNET_POOL_FILE, "w") as f:
        json.dump(data, f, indent=2, ensure_ascii=False)

def subnet_get_all_ips(subnet_str: str, skip: list) -> list:
    """Return all host IPs from a /24 subnet, skipping indices in skip list.
    Example: subnet='85.93.5.0/24', skip=[0,255] → ['85.93.5.1',...,'85.93.5.254']"""
    base = subnet_str.split('/')[0]
    parts = base.split('.')
    prefix = '.'.join(parts[:3])
...
def subnet_switch_ip(pool_data: dict, new_ip: str) -> Tuple[bool, str]:
    """Switch GOST to new subnet IP via SSH to VPS.
    1. ip addr add new_ip/32
    2. Rewrite gost.service
    3. daemon-reload + restart gost
    4. Verify gost is active
    Returns (success, message)."""
    vps_ip = pool_data.get('vps_ip', '')
    vps_port = pool_data.get('vps_port', 22)
    vps_user = pool_data.get('vps_user', 'root')
    vps_pass = pool_data.get('vps_pass', '')
    ...
    # 1. Add IP to interface (ignore error if already exists)
    cmd_add = f"ip addr add {new_ip}/32 dev {iface} 2>/dev/null ; echo IPADD_DONE"
    ok, out = ssh_exec(vps_ip, vps_port, vps_user, vps_pass, cmd_add)
    if not ok:
    ...
    cmd_gost = (
        f'printf "{service_content}\\n" > /usr/lib/systemd/system/gost.service && '
        f'systemctl daemon-reload && systemctl restart gost && sleep 1 && '
        f'systemctl is-active gost'
    )
    ok2, out2 = ssh_exec(vps_ip, vps_port, vps_user, vps_pass, cmd_gost)
    if not ok2:
        return False, f"SSH ошибка (gost restart): {out2}"
    if 'active' not in out2.lower():
        return False, f"GOST не запустился: {out2}"

    # 3. Update pool data
    old_ip = pool_data.get('current_ip', '')
    pool_data['current_ip'] = new_ip
    if new_ip not in pool_data.get('used_ips', []):
        pool_data.setdefault('used_ips', []).append(new_ip)
    save_subnet_pool(pool_data)

    # 4. Update RR current_vpn_ip.txt (+ pptp if pptp toggle on)
    try:
    ...
    return True, f"✅ GOST переключен: {old_ip} → {new_ip}"
...
TUNNEL_TIMEOUT = 600  # 10 minutes
# Active tunnels: {cn: {port, ssh_client, server_sock, stop_event, thread, close_task, started, front_ip}}
...
# =====================================================================
#  SSH TUNNEL — local port forwarding via paramiko
# =====================================================================
def _tunnel_alloc_port() -> Optional[int]:
    """Find a free port in the tunnel range."""
    used = {t["port"] for t in _active_tunnels.values()}
...
def _tunnel_forward_handler(conn, transport, remote_host, remote_port):
    """Handle one forwarded TCP connection through the SSH tunnel."""
    try:
    ...
    try:
        while True:
            r, _, _ = select.select([conn, chan], [], [], 1.0)
            if conn in r:
                data = conn.recv(4096)
                if not data:
                    break
...
def _tunnel_accept_loop(server_sock, transport, remote_host, remote_port, stop_event):
    """Accept incoming connections and forward them through SSH."""
    ...
    while not stop_event.is_set():
        try:
            ...
            break
        t = threading.Thread(target=_tunnel_forward_handler,
                             args=(conn, transport, remote_host, remote_port),
                             daemon=True)
        t.start()


def _tunnel_start_local_fwd(ssh_client, local_port, remote_host="127.0.0.1", remote_port=80):
    """Start a local port forwarding server. Returns (server_sock, stop_event, thread)."""
    ...
    thread = threading.Thread(target=_tunnel_accept_loop,
                              args=(server_sock, transport, remote_host, remote_port, stop_event),
                              daemon=True)
    thread.start()
    return server_sock, stop_event, thread


def _tunnel_connect_router(cn: str):
    """Create SSH connection to a router (direct or via jump host). Returns paramiko.SSHClient or raises."""
    routers = load_routers()
    r = routers.get(cn)
    if not r:
        raise ValueError(f"Роутер {cn} не найден")
    ...
    if is_pptp:
        ...
        if not pptp_ip:
            raise ValueError(f"PPTP IP не найден для {cn}")
        srv = load_pptp_server()
        if not srv.get("host"):
            raise ValueError("PPTP сервер не настроен")
        # Connect via jump host
        jump_client = paramiko.SSHClient()
        jump_client.set_missing_host_key_policy(paramiko.AutoAddPolicy())
        jump_client.connect(srv["host"], port=int(srv.get("port", 22)),
                            username=srv.get("user", "root"), password=srv.get("password", ""),
                            timeout=SSH_TIMEOUT, look_for_keys=False, allow_agent=False)
        transport = jump_client.get_transport()
        channel = transport.open_channel("direct-tcpip", (pptp_ip, port),
                                          ("127.0.0.1", 0), timeout=SSH_TIMEOUT)
        target_client = paramiko.SSHClient()
        target_client.set_missing_host_key_policy(paramiko.AutoAddPolicy())
        target_client.connect(pptp_ip, port=port, username=user, password=password,
                              sock=channel, timeout=SSH_TIMEOUT,
                              look_for_keys=False, allow_agent=False)
        # Store jump_client on target so it stays alive
        target_client._tunnel_jump = jump_client
        return target_client
    else:
        ip = get_router_ip(cn)
        if not ip:
            raise ValueError(f"VPN IP не найден для {cn}")
        client = paramiko.SSHClient()
        client.set_missing_host_key_policy(paramiko.AutoAddPolicy())
        client.connect(ip, port=port, username=user, password=password,
                       timeout=SSH_TIMEOUT, look_for_keys=False, allow_agent=False)
...
def _tunnel_stop(cn: str):
    """Stop an active tunnel and clean up."""
    info = _active_tunnels.pop(cn, None)
    if not info:
        return
    # Stop accept loop
    ...
    if stop_event:
        stop_event.set()
    # Close server socket
    server_sock = info.get("server_sock")
    if server_sock:
        try:
            server_sock.close()
        except Exception:
            pass
    # Close SSH to router
    ssh_client = info.get("ssh_client")
    if ssh_client:
        jump = getattr(ssh_client, '_tunnel_jump', None)
        try:
            ssh_client.close()
        except Exception:
            pass
        if jump:
            try:
                ...
                pass
    # Kill temporary gost process on the front
    front_ip = info.get("front_ip")
    front_info = info.get("front_info")
    port = info.get("port")
    if front_ip and front_info and port:
        try:
            kill_cmd = f"pkill -f 'gost.*:{port}/'"
            ssh_exec(front_ip, 22, front_info["ssh_user"], front_info["ssh_pass"], kill_cmd)
        except Exception:
            pass
    # Cancel auto-close task
    close_task = info.get("close_task")
    if close_task and not close_task.done():
        close_task.cancel()


# =====================================================================
#  SSH TUNNEL — Bot handlers
# =====================================================================
async def tunnel_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Show tunnel management menu."""
    q = update.callback_query
    await q.answer()
    active = len(_active_tunnels)
    lines = []
    if _active_tunnels:
        for cn, info in _active_tunnels.items():
            elapsed = int(time.time() - info["started"])
            remaining = max(0, TUNNEL_TIMEOUT - elapsed)
            lines.append(f"  🟢 <b>{cn}</b> → порт {info['port']}  ({remaining // 60}:{remaining % 60:02d})")
    kb = [
        [InlineKeyboardButton("🔓 Открыть туннель", callback_data='tunnel_open')],
    ]
    if _active_tunnels:
        kb.append([InlineKeyboardButton("🔒 Закрыть туннель", callback_data='tunnel_close_select')])
        kb.append([InlineKeyboardButton("🔒 Закрыть все", callback_data='tunnel_close_all')])
    kb.append([InlineKeyboardButton("◀️ Назад", callback_data='ssh_routers')])
    text = f"🖥 <b>Админка роутера (SSH-туннель)</b>\n\nАктивных: {active}"
    if lines:
        text += "\n" + "\n".join(lines)
    text += (
        "\n\n<i>Туннель открывает доступ к веб-интерфейсу\n"
        f"роутера через GOST-фронт на {TUNNEL_TIMEOUT // 60} мин.</i>"
    )
    await safe_edit_text(q, context, text, parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup(kb))


async def tunnel_open_select(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Show online routers to open a tunnel to."""
    q = update.callback_query
    await q.answer()
    routers = load_routers()
    if not routers:
        await safe_edit_text(q, context, "Нет роутеров. Добавьте через ➕ Добавить.",
...
async def tunnel_open_start(update: Update, context: ContextTypes.DEFAULT_TYPE, cn: str, front_ip: str):
    ...
    if not port:
        await safe_edit_text(q, context,
            "❌ Нет свободных портов (все туннели заняты).",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("◀️ Назад", callback_data='tunnel_menu')]]))
        return
    msg = await safe_edit_text(q, context, f"⏳ Подключаюсь к {cn}...")
    try:
        # 1. SSH to router (blocking → run in thread)
        ssh_client = await asyncio.to_thread(_tunnel_connect_router, cn)
        # 2. Start local port forwarding
        server_sock, stop_event, thread = _tunnel_start_local_fwd(ssh_client, port)
        # 3. Start temporary gost on the front server
        # Use the backend's IP (where this bot runs) for the forward target
        backend_ip = "95.141.35.71"
        gost_cmd = (
            f"nohup /usr/local/bin/gost -L=tcp://:{port}/{backend_ip}:{port} "
            f">/dev/null 2>&1 & echo GOST_TMP_OK"
        )
        ok, out = await asyncio.to_thread(
            ssh_exec, front_ip, 22, front_info["ssh_user"], front_info["ssh_pass"], gost_cmd)
        if not ok or "GOST_TMP_OK" not in out:
            # Clean up
            stop_event.set()
            server_sock.close()
            ssh_client.close()
            await safe_edit_text(q, context,
                f"❌ Не удалось запустить GOST на фронте:\n<pre>{escape(out[:500])}</pre>",
                parse_mode="HTML",
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("◀️ Назад", callback_data='tunnel_menu')]]))
            return
        # 4. Schedule auto-close
        async def _auto_close():
            await asyncio.sleep(TUNNEL_TIMEOUT)
            _tunnel_stop(cn)
            try:
                await context.bot.send_message(
                    chat_id=ADMIN_ID,
                    text=f"🔒 Туннель <b>{cn}</b> закрыт (таймаут {TUNNEL_TIMEOUT // 60} мин).",
                    parse_mode="HTML")
            except Exception:
                pass
        close_task = asyncio.create_task(_auto_close())
        # 5. Save state
        _active_tunnels[cn] = {
            "port": port,
        ...
        await safe_edit_text(q, context,
            f"✅ <b>Туннель открыт!</b>\n\n"
            f"Роутер: <b>{cn}</b>\n"
            f"Админка: <code>http://{front_ip}:{port}</code>\n"
            f"⏱ Закроется через {TUNNEL_TIMEOUT // 60} мин.\n\n"
            f"<i>Откройте ссылку в браузере для доступа к настройкам роутера.</i>",
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("🔒 Закрыть сейчас", callback_data=f'tunnel_close:{cn}')],
                [InlineKeyboardButton("◀️ Назад", callback_data='tunnel_menu')]]))
    except Exception as e:
        await safe_edit_text(q, context,
...
async def tunnel_close_select(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Show active tunnels to close."""
    q = update.callback_query
    await q.answer()
    if not _active_tunnels:
        await safe_edit_text(q, context, "Нет активных туннелей.",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("◀️ Назад", callback_data='tunnel_menu')]]))
        return
    kb = []
    for cn, info in _active_tunnels.items():
        ...
        kb.append([InlineKeyboardButton(
            f"🔒 {cn} (порт {info['port']}, {remaining // 60}:{remaining % 60:02d})",
            callback_data=f'tunnel_close:{cn}')])
    kb.append([InlineKeyboardButton("◀️ Назад", callback_data='tunnel_menu')])
    await safe_edit_text(q, context,
        "🔒 <b>Закрыть туннель</b>\nВыберите:",
        parse_mode="HTML", reply_markup=InlineKeyboardMarkup(kb))
...
async def tunnel_close_handler(update: Update, context: ContextTypes.DEFAULT_TYPE, cn: str):
    """Close a specific tunnel."""
    q = update.callback_query
    await q.answer()
    if cn not in _active_tunnels:
        await safe_edit_text(q, context, "Туннель уже закрыт.",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("◀️ Назад", callback_data='tunnel_menu')]]))
        return
    port = _active_tunnels[cn]["port"]
    await asyncio.to_thread(_tunnel_stop, cn)
    await safe_edit_text(q, context,
        f"🔒 Туннель <b>{cn}</b> (порт {port}) закрыт.",
        parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("◀️ Назад", callback_data='tunnel_menu')]]))


async def tunnel_close_all_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Close all active tunnels."""
    q = update.callback_query
    await q.answer()
    count = len(_active_tunnels)
    for cn in list(_active_tunnels.keys()):
        _tunnel_stop(cn)
    await safe_edit_text(q, context,
        f"🔒 Закрыто туннелей: {count}",
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("◀️ Назад", callback_data='tunnel_menu')]]))
...
async def rr_set_ip_receive(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if auto_ip_state.get("pptp"):
        ...
        targets.append("PPTP")
    rr_append_history(f"IP changed: {old_ip} -> {text} ({'+'.join(targets)})")
    context.user_data.pop('await_rr_ip', None)
    await update.message.reply_text(f"✅ IP обновлён: <code>{text}</code> ({'+'.join(targets)})", parse_mode="HTML")
...
# =====================================================================
#  CHANGE PORT — Set new port for routers (via file, picked up by cron)
# =====================================================================
async def change_port_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query
    await q.answer()
    current_port = rr_read_file(RR_PORT_FILE, "(не задан)")
    port_scan = rr_read_flag(RR_PORT_SCAN_FLAG)
    scan_status = "🔴 выкл" if port_scan else "🟢 вкл"
    context.user_data['await_change_port'] = True
    await safe_edit_text(q, context,
        f"🔌 <b>Смена порта OpenVPN</b>\n\n"
        f"Текущий порт: <code>{current_port}</code>\n"
        f"Port Scan: {scan_status}\n\n"
        f"Введите новый порт (1-65535).\n"
        f"<i>Роутеры подхватят через крон (Port Scan должен быть вкл).</i>",
        parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("❌ Отмена", callback_data="rr_cancel")]]))
...
async def change_port_receive(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    old_port = rr_read_file(RR_PORT_FILE, "")
    rr_write_file(RR_PORT_FILE, str(new_port))
    rr_append_history(f"PORT: {old_port} -> {new_port}")
    port_scan = rr_read_flag(RR_PORT_SCAN_FLAG)
    if port_scan:
        hint = "⚠️ Port Scan выключен — роутеры не подхватят порт автоматически."
    else:
        hint = "✅ Port Scan включён — роутеры подхватят через крон (~5 мин)."
    kb = [[InlineKeyboardButton("🏠 Меню", callback_data='home')]]
    await update.message.reply_text(
        f"🔌 Порт обновлён: <code>{old_port}</code> → <code>{new_port}</code>\n\n{hint}",
        parse_mode="HTML", reply_markup=InlineKeyboardMarkup(kb))
...
def read_protocol() -> str:
    """Read current OpenVPN protocol from protocol.txt (udp/tcp)."""
    try:
        with open(RR_PROTOCOL_FILE, "r") as f:
            val = f.read().strip().lower()
            return val if val in ("udp", "tcp") else "udp"
    except FileNotFoundError:
        return "udp"

def write_protocol(proto: str):
    with open(RR_PROTOCOL_FILE, "w") as f:
        f.write(proto.strip().lower() + "\n")

async def protocol_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query
    await q.answer()
    cur = read_protocol()
    kb = [
        [InlineKeyboardButton(
            f"{'✅' if cur == 'udp' else '⬜'} UDP",
            callback_data='proto_set_udp'),
         InlineKeyboardButton(
            f"{'✅' if cur == 'tcp' else '⬜'} TCP",
            callback_data='proto_set_tcp')],
        [InlineKeyboardButton("🏠 Меню", callback_data='home')],
    ]
    await safe_edit_text(q, context,
        f"🔀 <b>Протокол OpenVPN</b>\n\n"
        f"Текущий: <code>{cur.upper()}</code>\n\n"
        f"Роутеры подхватят новый протокол в следующем цикле (~15 мин).\n"
        f"Протокол пишется в extended config: <code>remote IP PORT {cur}</code>",
        parse_mode="HTML", reply_markup=InlineKeyboardMarkup(kb))

async def proto_set(update: Update, context: ContextTypes.DEFAULT_TYPE, proto: str):
    q = update.callback_query
    await q.answer()
    old = read_protocol()
    write_protocol(proto)
    rr_append_history(f"PROTO: {old} -> {proto}")
    kb = [[InlineKeyboardButton("🔀 Протокол", callback_data='protocol_menu')],
          [InlineKeyboardButton("🏠 Меню", callback_data='home')]]
    await safe_edit_text(q, context,
        f"✅ Протокол изменён: <code>{old.upper()}</code> → <code>{proto.upper()}</code>",
        parse_mode="HTML", reply_markup=InlineKeyboardMarkup(kb))
...
def read_pptp_ip() -> str:
    try:
        with open(RR_PPTP_IP_FILE, "r") as f:
            return f.read().strip()
    except FileNotFoundError:
        return ""

def write_pptp_ip(ip: str):
    with open(RR_PPTP_IP_FILE, "w") as f:
        f.write(ip.strip() + "\n")
...
async def pptp_set_ip_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query
    await q.answer()
    context.user_data['await_pptp_ip'] = True
    cur = read_pptp_ip() or "(не задан)"
    await safe_edit_text(q, context,
...
async def pptp_set_ip_receive(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    old = read_pptp_ip()
    write_pptp_ip(ip)
    rr_append_history(f"PPTP_IP: {old} -> {ip}")
    kb = [[InlineKeyboardButton("🔗 PPTP", callback_data='pptp_menu')],
          [InlineKeyboardButton("🏠 Меню", callback_data='home')]]
    await update.message.reply_text(
...
async def emg_apply_one(update: Update, context: ContextTypes.DEFAULT_TYPE, cn: str):
    ...
    # Get first domain from domain_list for wget
    domains_raw = rr_read_file(RR_DOMAIN_LIST_FILE, "").strip()
    first_domain = ""
    for line in domains_raw.splitlines():
        ...
        if line and not line.startswith("#"):
            ...
            break
    if not first_domain:
        await safe_edit_text(q, context, "❌ Нет доменов в domain_list.txt")
        return
    cmd = (
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # --- Subnet Pool ---
    elif data == 'subnet_menu':
        await subnet_menu(update, context)
    elif data == 'subnet_next':
        await subnet_next(update, context)
    elif data == 'subnet_next_confirm':
        await subnet_next_confirm(update, context)
    elif data == 'subnet_status':
        await subnet_status(update, context)
    elif data == 'subnet_setup':
        await subnet_setup_start(update, context)
    elif data == 'subnet_block_current':
        await subnet_block_current(update, context)
    elif data.startswith('subnet_block:'):
        ip = data[len('subnet_block:'):]
        await subnet_block_ip(update, context, ip)
    elif data == 'subnet_unblock':
        await subnet_unblock(update, context)
    elif data.startswith('subnet_ub:'):
        ip = data[len('subnet_ub:'):]
        await subnet_unblock_ip(update, context, ip)
    elif data.startswith('subnet_set:'):
        field = data[len('subnet_set:'):]
        await subnet_setup_field(update, context, field)
    elif data == 'subnet_history':
    ...
    # --- Tunnel callbacks ---
        ...
        await tunnel_close_handler(update, context, data[len('tunnel_close:'):])
    elif data == 'tunnel_close_all':
        await tunnel_close_all_handler(update, context)
    elif data == 'ssh_list':
    ...
    elif data == 'protocol_menu':
        await protocol_menu(update, context)
    elif data == 'proto_set_udp':
        await proto_set(update, context, 'udp')
...
async def ssh_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb = [
        ...
        [InlineKeyboardButton("🖥 Админка роутера", callback_data='tunnel_menu')],
        [InlineKeyboardButton("🏠 В главное меню", callback_data='home')],
    ]
    await safe_edit_text(q, context,
        "🖥 <b>SSH Роутеры</b>\n\n"
...
# =====================================================================
#  SUBNET POOL — menu & handlers
# =====================================================================

async def subnet_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query
    pool = load_subnet_pool()
    if not pool or not pool.get('subnet'):
        kb = [
            [InlineKeyboardButton("⚙️ Настроить", callback_data='subnet_setup')],
            [InlineKeyboardButton("◀️ Назад", callback_data='home')],
        ]
        await safe_edit_text(q, context,
            "🌐 <b>Subnet Pool</b>\n\nНе настроен. Нажмите «Настроить» для начала.",
            parse_mode="HTML", reply_markup=InlineKeyboardMarkup(kb))
        return
    subnet = pool.get('subnet', '?')
    current = pool.get('current_ip', '—')
    all_ips = subnet_get_all_ips(subnet, pool.get('skip_ips', [0, 255]))
    blocked = pool.get('blocked_ips', [])
    free = len([ip for ip in all_ips if ip not in blocked and ip != current])
    kb = [
        [InlineKeyboardButton("➡️ Следующий IP", callback_data='subnet_next')],
        [InlineKeyboardButton("📋 Статус", callback_data='subnet_status')],
        [InlineKeyboardButton("🚫 Заблокировать текущий", callback_data='subnet_block_current')],
        [InlineKeyboardButton("🔓 Разблокировать IP", callback_data='subnet_unblock')],
        [InlineKeyboardButton("📜 История", callback_data='subnet_history')],
        [InlineKeyboardButton("⚙️ Настройка", callback_data='subnet_setup')],
        [InlineKeyboardButton("◀️ Назад", callback_data='home')],
    ]
    await safe_edit_text(q, context,
        f"🌐 <b>Подсеть {subnet}</b>\n\n"
        f"Текущий IP: <code>{current}</code>\n"
        f"Использовано: {len(pool.get('used_ips', []))}\n"
        f"Заблокировано: {len(blocked)}\n"
        f"Свободно: {free}",
        parse_mode="HTML", reply_markup=InlineKeyboardMarkup(kb))


async def subnet_next(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query
    pool = load_subnet_pool()
    nxt = subnet_next_ip(pool)
    if not nxt:
        await safe_edit_text(q, context,
            "🚨 Все IP подсети использованы или заблокированы!",
            reply_markup=InlineKeyboardMarkup(
                [[InlineKeyboardButton("◀️ Назад", callback_data='subnet_menu')]]))
        return
    current = pool.get('current_ip', '—')
    context.user_data['subnet_next_ip'] = nxt
    kb = [
        [InlineKeyboardButton("✅ Да, переключить", callback_data='subnet_next_confirm')],
        [InlineKeyboardButton("❌ Отмена", callback_data='subnet_menu')],
    ]
    await safe_edit_text(q, context,
        f"➡️ Переключить GOST?\n\n"
        f"Текущий: <code>{current}</code>\n"
        f"Новый: <code>{nxt}</code>\n\n"
        f"Пометить <code>{current}</code> как заблокированный?",
        parse_mode="HTML", reply_markup=InlineKeyboardMarkup(kb))


async def subnet_next_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query
    new_ip = context.user_data.pop('subnet_next_ip', None)
    if not new_ip:
        await safe_edit_text(q, context, "Ошибка: IP не выбран.")
        return
    pool = load_subnet_pool()
    old_ip = pool.get('current_ip', '')
    await safe_edit_text(q, context,
        f"⏳ Переключаю GOST: {old_ip} → {new_ip}...", parse_mode="HTML")
    ok, msg = await asyncio.to_thread(subnet_switch_ip, pool, new_ip)
    kb = [
        [InlineKeyboardButton("◀️ Меню подсети", callback_data='subnet_menu')],
    ]
    if ok:
        # Ask whether to block old IP
        if old_ip:
            kb.insert(0, [InlineKeyboardButton(
                f"🚫 Заблокировать {old_ip}", callback_data=f'subnet_block:{old_ip}')])
        await q.message.edit_text(
            f"{msg}\n\nGOST слушает на <code>{new_ip}</code>",
            parse_mode="HTML", reply_markup=InlineKeyboardMarkup(kb))
    else:
        await q.message.edit_text(
            f"❌ Ошибка: {msg}",
...
async def subnet_status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query
    pool = load_subnet_pool()
    subnet = pool.get('subnet', '?')
    current = pool.get('current_ip', '—')
    all_ips = subnet_get_all_ips(subnet, pool.get('skip_ips', [0, 255]))
    blocked = pool.get('blocked_ips', [])
    used = pool.get('used_ips', [])
    free = len([ip for ip in all_ips if ip not in blocked and ip != current])
    vps_ip = pool.get('vps_ip', '')
    vps_port = pool.get('vps_port', 22)
    vps_user = pool.get('vps_user', 'root')
    vps_pass = pool.get('vps_pass', '')

    text = (
        f"📋 <b>Статус Subnet Pool</b>\n\n"
        f"Подсеть: <code>{subnet}</code>\n"
        f"VPS: <code>{vps_ip}</code>\n"
        f"Backend: <code>{pool.get('gost_backend', '?')}</code>\n"
        f"Текущий IP: <code>{current}</code>\n\n"
        f"Всего: {len(all_ips)} | Использовано: {len(used)} | "
        f"Заблокировано: {len(blocked)} | Свободно: {free}\n"
    )
    if blocked:
        text += "\n🚫 Заблокированные:\n" + "\n".join(
            f"  • <code>{ip}</code>" for ip in blocked[-10:])
        if len(blocked) > 10:
            text += f"\n  ...и ещё {len(blocked)-10}"

    # Check GOST status on VPS
    if vps_ip and vps_pass:
        ok, out = await asyncio.to_thread(
            ssh_exec, vps_ip, vps_port, vps_user, vps_pass,
            "systemctl is-active gost && echo GOST_PORT: $(ss -ulnp 2>/dev/null | grep gost | head -1)")
        if ok:
            text += f"\n\n🔧 GOST: <pre>{escape(out[:300])}</pre>"

    kb = [[InlineKeyboardButton("◀️ Назад", callback_data='subnet_menu')]]
    await safe_edit_text(q, context, text, parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup(kb))


async def subnet_block_current(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query
    pool = load_subnet_pool()
    current = pool.get('current_ip', '')
    if not current:
        await safe_edit_text(q, context, "Нет текущего IP.")
        return
    blocked = pool.get('blocked_ips', [])
    if current in blocked:
        await safe_edit_text(q, context,
            f"<code>{current}</code> уже в списке заблокированных.",
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup(
                [[InlineKeyboardButton("◀️ Назад", callback_data='subnet_menu')]]))
        return
    blocked.append(current)
    pool['blocked_ips'] = blocked
    save_subnet_pool(pool)
    kb = [[InlineKeyboardButton("◀️ Назад", callback_data='subnet_menu')]]
    await safe_edit_text(q, context,
        f"🚫 <code>{current}</code> помечен как заблокированный.",
        parse_mode="HTML", reply_markup=InlineKeyboardMarkup(kb))


async def subnet_block_ip(update: Update, context: ContextTypes.DEFAULT_TYPE, ip: str):
    """Block a specific IP (called from callback subnet_block:x.x.x.x)."""
    q = update.callback_query
    pool = load_subnet_pool()
    blocked = pool.get('blocked_ips', [])
    if ip not in blocked:
        ...
        pool['blocked_ips'] = blocked
        save_subnet_pool(pool)
    kb = [[InlineKeyboardButton("◀️ Меню подсети", callback_data='subnet_menu')]]
    await safe_edit_text(q, context,
        f"🚫 <code>{ip}</code> заблокирован.",
        parse_mode="HTML", reply_markup=InlineKeyboardMarkup(kb))


async def subnet_unblock(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query
    pool = load_subnet_pool()
    blocked = pool.get('blocked_ips', [])
    if not blocked:
        await safe_edit_text(q, context, "Нет заблокированных IP.",
            reply_markup=InlineKeyboardMarkup(
                [[InlineKeyboardButton("◀️ Назад", callback_data='subnet_menu')]]))
        return
    kb = []
    for ip in blocked:
        kb.append([InlineKeyboardButton(f"🔓 {ip}", callback_data=f'subnet_ub:{ip}')])
    kb.append([InlineKeyboardButton("◀️ Назад", callback_data='subnet_menu')])
    await safe_edit_text(q, context,
        "🔓 Выберите IP для разблокировки:",
        reply_markup=InlineKeyboardMarkup(kb))


async def subnet_unblock_ip(update: Update, context: ContextTypes.DEFAULT_TYPE, ip: str):
    q = update.callback_query
    pool = load_subnet_pool()
    blocked = pool.get('blocked_ips', [])
    if ip in blocked:
    ...
    kb = [[InlineKeyboardButton("◀️ Назад", callback_data='subnet_menu')]]
    await safe_edit_text(q, context,
        f"🔓 <code>{ip}</code> разблокирован.",
        parse_mode="HTML", reply_markup=InlineKeyboardMarkup(kb))


async def subnet_history(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query
    pool = load_subnet_pool()
    used = pool.get('used_ips', [])
    blocked = set(pool.get('blocked_ips', []))
    current = pool.get('current_ip', '')
    if not used:
        text = "📜 История пуста."
    else:
        lines = []
        for i, ip in enumerate(used, 1):
            mark = "🟢" if ip == current else ("🚫" if ip in blocked else "⬜")
            lines.append(f"  {i}. {mark} <code>{ip}</code>")
        text = "📜 <b>История использования IP:</b>\n\n" + "\n".join(lines)
        text += f"\n\n🟢 = текущий, 🚫 = заблокирован"
    kb = [[InlineKeyboardButton("◀️ Назад", callback_data='subnet_menu')]]
    await safe_edit_text(q, context, text, parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup(kb))
...
# --- Subnet Setup Wizard ---
SUBNET_SETUP_FIELDS = [
    ('subnet', 'Подсеть (например 85.93.5.0/24)', None),
    ('vps_ip', 'IP адрес VPS', None),
    ('vps_port', 'SSH порт VPS', '22'),
    ('vps_user', 'SSH пользователь VPS', 'root'),
    ('vps_pass', 'SSH пароль VPS', None),
...
async def subnet_setup_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query
    pool = load_subnet_pool()
    if pool and pool.get('subnet'):
        # Show current config and offer to change individual fields
        lines = []
        for key, label, _ in SUBNET_SETUP_FIELDS:
            val = pool.get(key, '—')
            if key == 'vps_pass':
                val = '***' if val else '—'
        ...
        for key, label, _ in SUBNET_SETUP_FIELDS:
            ...
            kb.append([InlineKeyboardButton(f"✏️ {short}", callback_data=f'subnet_set:{key}')])
        kb.append([InlineKeyboardButton("◀️ Назад", callback_data='subnet_menu')])
        await safe_edit_text(q, context,
            "⚙️ <b>Настройка Subnet Pool</b>\n\n" + "\n".join(lines) +
            "\n\nНажмите на поле для изменения:",
            parse_mode="HTML", reply_markup=InlineKeyboardMarkup(kb))
    else:
        # First-time setup: start wizard
        context.user_data['subnet_setup_data'] = {}
        context.user_data['subnet_setup_step'] = 0
        await _subnet_setup_ask(q, context)


async def _subnet_setup_ask(q, context):
    """Ask the current wizard step question."""
    step = context.user_data.get('subnet_setup_step', 0)
    if step >= len(SUBNET_SETUP_FIELDS):
        # All done — save
        data = context.user_data.pop('subnet_setup_data', {})
        ...
        data.setdefault('skip_ips', [0, 255])
        save_subnet_pool(data)
        kb = [[InlineKeyboardButton("🌐 Меню подсети", callback_data='subnet_menu')]]
        await q.message.edit_text(
            "✅ Subnet Pool настроен!",
            reply_markup=InlineKeyboardMarkup(kb))
        return
    key, label, default = SUBNET_SETUP_FIELDS[step]
    context.user_data['await_subnet_setup'] = key
    hint = f"\n(по умолчанию: {default})" if default else ""
    cancel_kb = InlineKeyboardMarkup(
        [[InlineKeyboardButton("❌ Отмена", callback_data='subnet_menu')]])
    await q.message.edit_text(
        f"⚙️ Шаг {step+1}/{len(SUBNET_SETUP_FIELDS)}\n\n"
        f"Введите <b>{label}</b>:{hint}",
        parse_mode="HTML", reply_markup=cancel_kb)


async def subnet_setup_field(update: Update, context: ContextTypes.DEFAULT_TYPE, field: str):
    """Start editing a single field from the settings view."""
    q = update.callback_query
    pool = load_subnet_pool()
    for key, label, default in SUBNET_SETUP_FIELDS:
        if key == field:
            context.user_data['await_subnet_setup'] = key
            context.user_data['subnet_setup_single'] = True
            current_val = pool.get(key, '')
            if key == 'vps_pass':
                current_val = '***' if current_val else ''
            hint = f"\nТекущее: <code>{current_val}</code>" if current_val else ""
            if default:
                hint += f"\nПо умолчанию: {default}"
            cancel_kb = InlineKeyboardMarkup(
                [[InlineKeyboardButton("❌ Отмена", callback_data='subnet_setup')]])
            await safe_edit_text(q, context,
...
async def subnet_setup_receive(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle text input for subnet setup (wizard or single-field edit)."""
    key = context.user_data.get('await_subnet_setup')
    if not key:
        return False
    context.user_data.pop('await_subnet_setup', None)
    text = update.message.text.strip()

    # Find default for this key
    default = None
    for k, _, d in SUBNET_SETUP_FIELDS:
        if k == key:
            default = d
            break
    ...
    if context.user_data.get('subnet_setup_single'):
        ...
        # Wizard mode
        data = context.user_data.get('subnet_setup_data', {})
        data[key] = text
        context.user_data['subnet_setup_data'] = data
        step = context.user_data.get('subnet_setup_step', 0) + 1
        context.user_data['subnet_setup_step'] = step
        # Use a fake query object to reuse _subnet_setup_ask
        msg = await update.message.reply_text("⏳...")
        # Create a mini-object with .message for edit_text
        class _FakeQ:
            def __init__(self, m): self.message = m
        await _subnet_setup_ask(_FakeQ(msg), context)
        return True
...
🔀 Протокол
  Переключение OpenVPN между UDP и TCP.
  Глобальная настройка для всех роутеров.
  Протокол пишется в extended config:
  remote IP PORT udp/tcp
  Роутеры подхватят в следующем цикле.

🔗 PPTP
  Управление PPTP VPN (параллельно с OpenVPN).
  PPTP использует своё поле vpnc_peer (Remote
  VPN Server), а OpenVPN — только extended config.
  Нет конфликтов между протоколами.
  ✏️ Сменить IP — задать PPTP front IP
  🌐 Домены — управление PPTP domain list
  Ротация доменов работает как у OpenVPN.
  Скрипт определяет тип VPN (vpnc_type) и
  выбирает нужную логику автоматически.
  Padavan vpnc_type: 0=PPTP, 1=L2TP, 2=OpenVPN.

🔀 Переключатель VPN
  Переключение роутеров между OpenVPN и PPTP.
  Выбираешь роутер → тип VPN → бот меняет
  vpnc_type через SSH и перезапускает клиент.
  При переключении на OpenVPN бот восстанавливает
  ovpnc.script (PPTP удаляет его из RAM).
  Если роутер на PPTP — SSH идёт через jump host
  (PPTP сервер), т.к. PPTP IP (172.16.0.x)
  недоступен из OpenVPN сети (10.9.0.x).
  В статистике: 🟡 = PPTP, 🟢 = OpenVPN.
  Автокоррекция: если PPTP роутер виден в
  OpenVPN status.log → бот исправляет тип.

🚨 Экстренное меню
  Аварийные функции для массового воздействия
  БЕЗ SSH — через HTTP флаги на сервере.
  Роутеры проверяют флаги при каждом цикле (~15 мин).

  🛟 Аварийный конфиг (OEC)
    Extended config который роутеры скачивают
    и применяют принудительно при каждом цикле.
    Формат: строки «remote IP PORT proto»
    (как в обычном extended config).
    Работает даже если роутер недоступен по SSH.
    Сценарий: заблокировали IP → пишешь новый
    remote в OEC → роутеры сами переключатся.
    ✏️ Редактировать — задать/изменить OEC
    🔴 Вкл/Выкл — активировать/убрать emergency.flag
    📤 Разослать — принудительно применить через SSH
      на один / несколько / все роутеры сразу
    Когда emergency.flag включён — роутеры
    скачивают OEC КАЖДЫЙ цикл и перезаписывают
    конфиг. Выключи флаг когда ситуация стабильна.

  🔀 PPTP→OVPN аварийка
    Если провайдер заглушил PPTP — ставит флаг
    pptp_emergency.txt на сервере. Все PPTP роутеры
    при следующем цикле переключатся на OpenVPN
    автоматически (nvram + restart_vpn_client).
    Не требует SSH к каждому роутеру.
...
🌐 Подсеть
  Управление арендованной /24 подсетью
  для GOST-фронта. IP из подсети назначаются
  на VPS и используются как точка входа.

  ➡️ Следующий IP — переключить GOST на
    следующий свободный IP из подсети.
    Ищет после текущего, оборачивается в конце.
    Предлагает заблокировать старый IP.
  📋 Статус — текущий IP, счётчики
    (использовано / заблокировано / свободно),
    проверка GOST (systemctl is-active).
  🚫 Пометить заблокированным — добавить
    текущий IP в blocked_ips без переключения.
  📜 История — список использованных IP
    с пометками 🟢 текущий / 🚫 заблокирован.
  ⚙️ Настройка — wizard для настройки:
    подсеть, VPS SSH доступ, интерфейс,
    GOST backend/протокол/порт.
    Если уже настроено — редактирование
    отдельных полей.

  💡 При смене IP обновляется current_vpn_ip.txt
     чтобы роутеры подхватили новый адрес.
...
🔀 PPTP форвард (в разделе GOST)
  Проброс PPTP трафика через iptables на
  GOST-серверах. Установка/удаление правил,
  смена бэкенда (куда проксировать), статус.
  Позволяет скрыть реальный PPTP сервер
  за фронтом — аналогично GOST для OpenVPN.
...
Архитектура:
  ...
  Роутеры (cron каждые 15 мин)
    → Определяют тип VPN (vpnc_type):
      0=PPTP, 1=L2TP, 2=OpenVPN
    → OpenVPN: проверяют IP/порт с сервера,
      обновляют конфиг если изменилось
    → PPTP: ротация доменов, проверка туннеля
    → Самообновление: сверяют version.txt
    → Маяк: отмечаются на сервере (ID из
      /etc/storage/router_id или MAC)
    → Аварийки: OEC + PPTP→OVPN через HTTP флаги
...
При блокировке PPTP:
  1. Включаешь PPTP→OVPN аварийку в боте
  2. Сервер ставит флаг pptp_emergency.txt
  3. PPTP роутеры при следующем цикле
     переключаются на OpenVPN сами
"""
...
async def pptp_fwd_status(update: Update, context: ContextTypes.DEFAULT_TYPE, ip: str):
    ...
    status_cmd = (
        "echo '=== NAT PREROUTING ===' ; "
        "iptables -t nat -L PREROUTING -n -v 2>/dev/null | grep -E '(1723|gre|443)' ; "
        "echo '=== RAW PREROUTING ===' ; "
    ...
    saved_pptp = srv.get("pptp_backend", "не задан")
    saved_ovpn = srv.get("ovpn_backend", "не задан")
    if ok and "STATUS_OK" in out:
        has_pptp = "1723" in out and "gre" in out.lower()
        has_ovpn = "udp" in out.lower() and "443" in out
        pptp_icon = "🟢" if has_pptp else "🔴"
        ovpn_icon = "🟢" if has_ovpn else "🔴"
        result = (f"📊 <b>Форвард — {ip}</b>\n"
                  f"PPTP: {pptp_icon} {'Активен' if has_pptp else 'Не настроен'} → <code>{saved_pptp}</code>\n"
                  f"OpenVPN: {ovpn_icon} {'Активен' if has_ovpn else 'Не настроен'} → <code>{saved_ovpn}</code>\n\n"
